LLM (Large Language Model) - A neural network that is trained to predict the next term of and input sequence

Tokenization: When we give any prompt to LLM first it break the input in token i.e: All that glitters
...
5. Self Supervised learning:
    In this model will predicts thinks by its own i.e, if In photo I make one eye blank with some while square box and
    ask model to predict where the eye is looking, it would be the same direction in which other eye is looking.
 Another example of this if we give a sentence to model: The cat sat on the ___.
 In this it predict the last word by itself only