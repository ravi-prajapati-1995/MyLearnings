package com.ravi.learnings.dsa.binarySearch.on1DArray;
    ...
    public static int singleNonDuplicateOptimal(int[] nums) {
        ...
        while (low <= high) {
            ...
            if (nums[mid] != nums[mid + 1] &&
                    nums[mid] != nums[mid - 1]) {// meaning mid element don't have pair return it
                return nums[mid];
            }
            /*
            ...
            if ((mid % 2 == 0 && nums[mid] == nums[mid + 1]) ||
                    (mid % 2 == 1 && nums[mid] == nums[mid -
                            1])) { //we are on even index  and next element is same then we need to eliminatee
                low = mid + 1;
