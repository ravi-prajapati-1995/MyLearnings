...
public class LongestSubstringWithoutRepeatingCharacters {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(longestNonRepeatingSubstring_optimal("cadacz"));
    }

    /**
     * So this is the brute force solution,
     * we are using 2 loops basically we will create every possible pattern without
     * repeating character
     ...
     * TC - For each character we are traversing 2 time 1 in outer loop and 2nd in
     * inner loop so tc will be O(n^2) +
    ...
    public static int longestNonRepeatingSubstring(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            // This is the case when we don't have repeating character and inner loop didn't
            // break
            longest = Math.max(longest, temp);
        }
        return longest;
    ...
    /**
     * Instead of using hasset we can create a array of integer with 256 length as
     * we have only 256 characters
     * we will get ascii value for each character and check if the value is 1 or
     * not, if value is 1 then break the
     * inner loop
    ...
    /**
     * So to reduce the time complexity we will use the two pointer and sliding
     * window
     * 1. Create two pointer l and r starts from 0 and create a hashmap that will
     * stores the character and its last
     * position
     * 2. Then start a loop from 0 to length of string
     * 3.In the loop check if the current character exists in the Hashmap
     * 4. If Yes, then move the left pointer to the next where we get the current
     * character
     * 5. if No, then keep moving the right pointer r, and add each character in map
    ...
    public static int longestNonRepeatingSubstring_optimal(String s) {
        ...
        for (int i = 0; i < charArray.length; i++) {
            ...
            int length = r - l; // because if r and l are at same place then length will be 1
            if (length < 0) {
                continue;
            }
            if (l <= r) {
