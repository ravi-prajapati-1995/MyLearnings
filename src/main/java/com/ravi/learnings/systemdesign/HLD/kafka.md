...
Kafka main
We can have multiple replicas for a partition ie. P0 in Broker2, In case Broker1 is down
Leader: Partitions who are not replicas are called leaders, replicas known as followers,all read and write operations handle
by leader

What will happen when Machine size reaches while sotring the messages:
Ans: As there are multiple brokers running if one machine size is full message will move to another machine where different
partition of the machine will be there

What will happen if consumer goes Down?
Ans: If consumer goes down another consumer from same consumer group take over the and start processing messages from the offset
that is stored in zookeeper

What will happen when consumer will not able to process message?
Ans: When a message is not processed after defined retry count so that message will be moved into another queue failure
/dead letter queue. and committed count got increased

Kafka based on pull based approach, consumer will ask to kafka if there are any new message for me
RabbitMQ/ActiveMQ is Push based approach these MQ only will push message to the consumer